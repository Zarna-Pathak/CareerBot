...
with st.sidebar:
    st.header("Careerbot")
    if st.button("Cover Letter", key="sidebar_cover_letter"):
        st.session_state.page = "cover_letter"
    if st.button("Refined Resume", key="sidebar_refined_resume"):
        st.session_state.page = "refined_resume"
    if st.button("Rank Resume", key="sidebar_rank_resume"):
...
if st.session_state.page == "home":
        ...
        if st.session_state.cover_letter:
            ...
            with col1:
                if st.button("Generate a New Cover Letter"):
                    ...
                    st.rerun()
            with col2:
                docx_file=create_docx(st.session_state.get("cover_letter",""))
                st.download_button(
    ...
    if not os.getenv("GOOGLE_API_KEY"):
        ...
        if st.session_state.refined_resume:
            ...
            st.markdown(st.session_state.refined_resume, unsafe_allow_html=True)
        ...
        if st.button("Rank Resume"):
            if job_description and cv_file:
                ...
                if cv_text:
                    ...
                    st.markdown(ranking_result, unsafe_allow_html=True)
